package swdchatbox.modules.lecturer.controller;
...
import swdchatbox.modules.lecturer.dto.request.LecturerFilterRequest;
import swdchatbox.modules.lecturer.dto.request.LecturerRequest;
import swdchatbox.modules.lecturer.dto.request.LecturerUpdateRequest;
import swdchatbox.modules.lecturer.dto.response.LecturerResponse;
import swdchatbox.modules.lecturer.service.LecturerService;
...
public class LecturerController {
    ...
    @Operation(summary = "Cập nhật giảng viên", description = """
            Admin chỉnh sửa thông tin lecturer. `subjectIds` là danh sách môn đầy đủ sau cập nhật:
            gửi 3 ID → lecturer có đúng 3 môn; bớt 1 ID so với hiện tại → gỡ môn đó; môn đã có vẫn giữ nguyên.
            Tối thiểu 1 môn. Không gửi `subjectIds` thì không đổi môn.""")
    @PutMapping("/{id}")
